...
/*
		...
		01		24feb09	remove thumbnails view
...
{
	enum {
		LIST_BORDER = 2,	// border around list control
		TAB_BORDER = 2		// space between tabs and list
	};
	CRect	br, tr;	// bar rect, tab rect
	GetClientRect(br);
	m_TabCtrl.GetItemRect(0, tr);
	m_TabCtrl.MoveWindow(br);
	br.DeflateRect(LIST_BORDER, LIST_BORDER);
	br.top += tr.Height() + TAB_BORDER;
	if (m_CurSel >= 0)
		GetCurList()->MoveWindow(br);
}
