...
/*	
        ...
        03      23oct08	customize
		04		11jan09	move batch processing here
...
public:
	...
	~CJobControlDlg();
	BOOL	Create(CRecorder *Recorder, UINT nIDTemplate, CWnd* pParentWnd = NULL);
...
// Attributes
	...
	bool	SetBatchMode(bool Enable);
	bool	IsPoster(int JobIdx) const;
...
// Operations
	...
	bool	EndJob(int Status, LPCTSTR ErrorMsg = NULL);
	bool	FinishJob();
	bool	SkipJob();
	bool	AbortJob();
...
// Member data
	...
	bool	m_WasAborted;		// if true, batch processing was aborted
	bool	m_BatchMode;		// true if running queued jobs
	bool	m_BatchStop;		// true if stopping job queue
	CJobInfo	m_PreJobInfo;	// app state before running batch jobs
	HWND	m_hPosterDlg;		// poster dialog window handle
...
inline bool CJobControlDlg::WasAborted() const
{
	return(m_WasAborted);
}

inline bool CJobControlDlg::GetBatchMode() const
{
	return(m_BatchMode);
}

inline bool CJobControlDlg::IsPoster(int JobIdx) const
{
	return(m_Job.m_Info[JobIdx].m_RecInfo.IsPoster());
}
