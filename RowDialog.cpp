...
/*
		...
		07		03mar09	in ReplaceRows, allow zero Rows arg
		08		03mar09	remove OnRowDialogTab
		09		03mar09	in ReplaceRows, replace MoveWindow with SetWindowPos
...
bool CRowDialog::ReplaceRows(int Rows)
{
	...
	for (int i = 0; i < Rows; i++) {
		...
		rp->GetWindowRect(r);
		ScreenToClient(r);
		rp->SetWindowPos(&CWnd::wndBottom, 
			m_Margin.left, m_Margin.top + Pos * r.Height(), 
			r.Width() + m_Margin.right, r.Height(), 0);
