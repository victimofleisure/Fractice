...
/*
		...
		03		19apr08	missing DPoint.h
		04		20apr08	in StrToVal, return false if conversion fails
...
#define STRTOVAL(type, fmt) case FIO_##type: return(_stscanf(Str, fmt, Val) == 1);
...
bool CFormatIO::StrToVal(int Type, LPCTSTR Str, void *Val)
{
	...
	case FIO_DPOINT:
		return(_stscanf(Str, _T("%lf %lf"), 
			&((DPOINT *)Val)->x, &((DPOINT *)Val)->y) == 2);
	STRTOVAL(SHORT,		_T("%hd"));
	STRTOVAL(USHORT,	_T("%hu"));
	case FIO_BYTE:
		{
			int	i;
			if (_stscanf(Str, _T("%d"), &i) != 1)
				return(FALSE);
